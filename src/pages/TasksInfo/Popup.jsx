...
const Popup = ({ taskRecord, onClose }) => {
  const [plotData, setPlotdata] = useState(taskRecord.linePlot.lineData.data);
  const [plotTimes, setPlotTimes] = useState(taskRecord.linePlot.lineData.time);

  const [peaksData, setPeaksData] = useState(taskRecord.linePlot.Peaks.data);
  const [peaksTimes, setPeaksTimes] = useState(taskRecord.linePlot.Peaks.time);

  const [valleysData, setValleysData] = useState(
    taskRecord.linePlot.Valleys.data,
  );
  const [valleysTimes, setValleysTimes] = useState(
    taskRecord.linePlot.Valleys.time,
  );
  ...
  const handleClickonPlot = data => {
    ...
    if (!isMarkUp) {
      ...
      if (data.points[0].data.name === 'peak values') {
        if (
          peaksData.some(e => e === data.points[0].y) &&
          peaksTimes.some(e => e === data.points[0].x)
        ) {
          setSelectedPoint(
      ...
      if (data.points[0].data.name === 'valley values') {
        if (
          valleysData.some(e => e === data.points[0].y) &&
          valleysTimes.some(e => e === data.points[0].x)
        ) {
          setSelectedPoint(
            handleSelectElementfromArray(
              valleysData,
        ...
        <div className="flex justify-center">
          {revision != 0 ? (
            <button
              type="submit"
              onClick={handleSave}
              className=" bg-menu px-2 w-36 h-10 text-white rounded-md mt-3 py-2 font-semibold hover:bg-sky-500"
            >
              Save Points
            </button>
          ) : (
