...
export class CanvasDrawer {
  ...
  getBoundingBoxForCurrentFrame() {
    const boxData = this.boundingBoxes?.find(
      box => box.frameNumber === this.currentFrame,
    );
  ...
  getCurrentLandMark() {
    ...
    if (currentTask) {
      ...
      if (this.landMarks) return this.landMarks[this.currentFrame - offSet];
    } else return null;
  }
        ...
        if (Array.isArray(landMark[0])) {
          for (let mark of landMark) {
            ...
            this.canvasContext.arc(
              mark[0] + boundingBox.x,
              mark[1] + boundingBox.y,
              12,
              0,
              Math.PI * 2,
            );
          ...
          this.canvasContext.fillRect(
            landMark[0] + boundingBox.x - 25,
            landMark[1] + boundingBox.y - 25,
            50,
            50,
          );
          this.canvasContext.stroke();
    ...
    if (this.screen === 'tasks') {
      ...
      if (boundingBoxData) {
        boundingBoxData.forEach(box => {
          ...
          this.canvasContext.beginPath();
          this.canvasContext.strokeStyle = this.checkIfSubject(box.id)
            ? 'green'
            : 'red';
          this.canvasContext.lineWidth = 8;
          this.canvasContext.rect(box.x, box.y, box.width, box.height);
  ...
  getFrameNumber(timestamp) {
    return (
      Math.floor(timestamp * this.fps) +
      (this.frameOffset ? this.frameOffset : 0)
    );
  }
  ...
  handleDragStart(e) {
    ...
    const handleDrag = event => {
      ...
      const maxLeft = Math.max(
        (canvasElement.offsetWidth * this.zoomLevel - containerWidth) / 2,
        0,
      );
      const minLeft = Math.min(
        0,
        (containerWidth - canvasElement.offsetWidth * this.zoomLevel) / 2,
      );
      const maxTop = Math.max(
        (canvasElement.offsetHeight * this.zoomLevel - containerHeight) / 2,
        0,
      );
      const minTop = Math.min(
        0,
        (containerHeight - canvasElement.offsetHeight * this.zoomLevel) / 2,
      );
  ...
  resizeTaskBox(resizedTaskBox) {
    const updatedTaskBoxes = this.taskBoxes.map(taskBox => {
      if (taskBox.id === resizedTaskBox.id) return resizedTaskBox;
      else return taskBox;
    });
  ...
  getXY(e) {
    ...
    return [
      (e.clientX - offsetX) * widthRatio,
      (e.clientY - offsetY) * heightRatio,
    ];
  }
